...
const DeleteWorkspaceModal: FC<DeleteWorkspaceModalProps> = (): JSX.Element => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      setLoading(true);
      const response = await deleteWorkspaceServerAction({
        workspaceId: workspace.id,
      });
      if (!response.success) return toast.error(response.message);
      if (response.workspace) {
        toast.success(response.message);
        onOpen("deleteWorkspace", { workspace: response.workspace });
      }
